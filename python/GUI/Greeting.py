...
class Greeting(tk.Frame):
    def __init__(self, master, controller, alpha, beta, gamma, delta, epsilon):
        ...
        ttk.Label(settings_frame, text='epsilon: ').grid(column=0, row=3)
        delta = self.get_entry(settings_frame, delta)
        delta.grid(column=1, row=3)

        ttk.Label(settings_frame, text='delta: ').grid(column=0, row=4)
        ...
        early_stopping = ttk.Combobox(settings_frame, values=[True, False])
